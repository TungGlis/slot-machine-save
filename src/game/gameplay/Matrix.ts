...
export class Matrix extends vbContainer {
    ...
    constructor() {
        ...
        for (i = 0; i < 7; i++) {
            ...
            for (j = 0; j < 5; j++) {
                ...
                this.screenMatrix[i][j].position.set(j * (this.screenX / 5)+100, i * (this.screenY / 3) - (this.screenY))
    ...
    RandomPress() {
        ...
        for (let i = 0; i < 7; i++) {
            for (let j = 0; j < 5; j++) {
                const test = this.tweens.create(`${i}${j}test`, this.screenMatrix[i][j], {y: this.screenMatrix[i][j].position.y - 20}, 500).easing(Easing.Cubic.Out).on('end', ()=> {
                    this.tweens.create(`${i}${j}test1`, this.screenMatrix[i][j], {y: this.screenMatrix[i][j].position.y + 20}, 200).easing(Easing.Cubic.In).on('end', ()=>{
                        this.MoveAnimation(step, this.screenMatrix, bottomIndex, i, j)
                    }).start()
                }).start()
            }
        }
    ...
    MoveAnimation(stepNo: number, target: vbImage[][], bottomIndex: number, i: number, j: number){
        stepNo -= 1
        this.tweens.create(`${i}${j}test2`, target[i][j], {y: target[i][j].position.y + (this.screenY/3)}, 300).on('end', ()=>{
            bottomIndex += 1
            if(bottomIndex >= 6){
                target[i][j].position.y -= this.screenY*2
            }
            if(stepNo <= 0){
